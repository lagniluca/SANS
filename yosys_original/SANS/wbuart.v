...
`ifdef	USE_LITE_UART
	rxuartlite	#(INITIAL_SETUP[23:0])
		rx(.i_clk(i_clk), .i_uart_rx(i_uart_rx), .o_wr(rx_stb), .o_data(rx_uart_data));
...
`else
	...
	rxuart	#(INITIAL_SETUP) rx(.i_clk(i_clk), .i_reset((i_rst)||(rx_uart_reset)),
			.i_setup(uart_setup), .i_uart_rx(i_uart_rx),
			.o_wr(rx_stb), .o_data(rx_uart_data), .o_break(rx_break),
			.o_parity_err(rx_perr), .o_frame_err(rx_ferr), .o_ck_uart(ck_uart));
...
`endif
	...
	ufifo	#(.LGFLEN(LCLLGFLEN), .RXFIFO(1))
		rxfifo(.i_clk(i_clk), .i_rst((i_rst)||(rx_break)||(rx_uart_reset)),
	...
	ufifo	#(.LGFLEN(LGFLEN), .RXFIFO(0))
		txfifo(.i_clk(i_clk), .i_rst((tx_break)||(tx_uart_reset)),
...
`else
	...
	txuart	#(INITIAL_SETUP) tx(.i_clk(i_clk), .i_reset(1'b0), .i_setup(uart_setup),
			.i_break(r_tx_break), .i_wr((tx_empty_n)), .i_data(tx_data),
			.i_cts_n(cts_n), .o_uart_tx(o_uart_tx), .o_busy(tx_busy));
`endif
